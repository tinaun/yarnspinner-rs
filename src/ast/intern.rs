...
//this might be completely unsound? idk it works for now
...
impl Interner {
    ...
    pub fn intern(&mut self, name: &str) -> InternedKey {
        if let Some(&id) = self.map.get(name) {
            return InternedKey(id);
        }
        let name = unsafe { self.alloc(name) };
        let id = InternedKey(self.map.len() as u32);
        self.map.insert(name, id.0);
        self.vec.push(name);
    ...
    pub fn lookup(&self, id: InternedKey) -> &str {
        let id = id.0;
